...
Static recompilation of **Mega Man X2 (USA)** built on
...
is known to hold for Mega Man X2.

What that means concretely:

* **Execution is LLE-first.** `src/x2_rtl.c` boots the real RESET vector
...
* **An IRQ is delivered only when `P.I` is clear.** `g_snes->inIrq` is the
  ...
  only takes it while interrupts are enabled. Mega Man X2 plants `NOP / BRA self`
...
The trace build carries a TCP debug server on port 4383 plus the
