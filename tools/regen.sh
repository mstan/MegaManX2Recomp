#!/usr/bin/env bash
# Regen pipeline driver for MegaManX2SNESRecomp.
#
# Regenerates src/gen/*.c from recomp/*.cfg over a verified Mega Man X2 (USA)
# ROM, then syncs recomp/funcs.h.
#
# ROM: mmx2.sfc at the repo root (headered .smc dumps are accepted — the loader
...
if [ ! -f "$ROM" ]; then
  echo "regen.sh: $ROM not found — stage the verified Mega Man X2 (USA) ROM at the repo root." >&2
  exit 1
fi
...
#
# Passed conditionally: v2_emit hard-errors on a missing manifest, and X2 has
