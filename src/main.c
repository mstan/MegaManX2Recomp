 ...
 * -DX2_RECOMP_UI=ON; its recomp_ui.cmake defines RECOMP_LAUNCHER. MMX drives
...
#define X2_WINDOW_TITLE    "Mega Man X2 (Recompiled)"
#define X2_GAME_NAME       "Mega Man X2"
#define X2_GAME_REGION     "(USA)"
#define X2_LAUNCHER_TITLE  "Mega Man X2 \xE2\x80\x94 Launcher"
#define X2_ROM_CRC32       0x947B0355u
...
static const char kWindowTitle[] = X2_WINDOW_TITLE;
...
static void X2Display_PreparePpuFrame(void) {
  ...
  int width = X2Display_ComputeFrameWidth(drawable_width, drawable_height,
                                           g_config.widescreen);
  g_snes_width = width;
...
bool X2Display_IsWidescreenEnabled(void) { return g_config.widescreen; }
...
void ChangeWindowScale(int scale_step) {
  ...
  if (SDL_GetDisplayUsableBounds(screen, &bounds) == 0) {
    ...
    int logical_width = X2Display_GetWindowBaseWidth(g_snes_width);
    int logical_height = X2Display_GetWindowBaseHeight();
  ...
  int w = new_scale * X2Display_GetWindowBaseWidth(g_snes_width);
  int h = new_scale * X2Display_GetWindowBaseHeight();
...
static uint16_t x2_runner_to_snes_joypad(uint16_t r) {
  uint16_t s = 0;
...
static void SdlRenderer_BeginDraw(int width, int height, uint8 **pixels, int *pitch) {
  ...
  X2Display_ComputeViewport(width, height, output_width, output_height,
...
int main(int argc, char** argv) {
  ...
  host_report_init(X2_GAME_NAME " " X2_GAME_REGION, SNESRECOMP_BUILD_VERSION);
  ...
  static char rom_path_buf[512];
  {
    ...
    static const uint8_t kX2RomSha256[32] = {
    ...
    if (!snesrecomp_launcher_resolve_rom_sha256(la_argc, la_argv, rom_path_buf,
                                                sizeof(rom_path_buf), kX2RomSha256)) {
  ...
  // Initialize debug server
  {
     ...
     * 4383 Mega Man X2. Distinct ports let sibling games run
     * concurrently on one host without TCP-bind collisions. */
    if (debug_server_init(4383) == 0) {
      fprintf(stderr, "[main] Debug server ready on port 4383\n");
    }
    if (start_paused) {
  ...
  g_snes_width = g_config.widescreen
      ? X2Display_ComputeFrameWidth(16, 9, true) : 256;
  ...
  int window_width = custom_size ? g_config.window_width :
      g_current_window_scale * X2Display_GetWindowBaseWidth(g_snes_width);
  int window_height = custom_size ? g_config.window_height :
      g_current_window_scale * X2Display_GetWindowBaseHeight();
  ...
  extern const RtlGameInfo kX2GameInfo;
  RtlRegisterGame(&kX2GameInfo);
...
error_reading:;
  ...
  while (running) {
    ...
    // use; the disabled path is only the save-state repros).
    {
      ...
      emu_oracle_run_frame(x2_runner_to_snes_joypad((uint16_t)(inputs & 0xFFF)),
                           x2_runner_to_snes_joypad((uint16_t)((inputs >> 12) & 0xFFF)));
    }
#endif
    ...
    if (!g_snes->disableRender) {
      ...
      /* Turbo (render skipped): the per-scanline pass (X2DrawPpuFrame /
