...
// X2SpcPlayer is now a minimal SPC-image loader: it allocates a 64KB
...
typedef struct X2SpcPlayer {
  SpcPlayer base;
  uint8 ram[65536];
} X2SpcPlayer;

static void Dsp_Write(X2SpcPlayer *p, uint8_t reg, uint8 value) {
  if (p->base.dsp)
...
static void Spc_Reset(X2SpcPlayer *p) {
...
static void SmwSpcPlayer_Initialize(SpcPlayer *p_in) {
  X2SpcPlayer *p = (X2SpcPlayer *)p_in;
...
SpcPlayer *SmwSpcPlayer_Create(void) {
  X2SpcPlayer *p = (X2SpcPlayer *)malloc(sizeof(X2SpcPlayer));
  memset(p, 0, sizeof(X2SpcPlayer));
